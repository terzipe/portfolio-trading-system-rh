...
def compute_thresholds(closes: list[float]) -> dict[float, float]:
    """Pure function, no I/O — percentile -> VIX-level threshold for each
    configured rung (config.VIX_PERCENTILE_RUNGS), given historical closes
    in any order. Independently unit-testable without hitting FRED."""
    if not closes:
        raise ValueError("compute_thresholds requires at least one close")
    ordered = sorted(closes)
    return {p: round(_percentile_value(ordered, p), 2) for p in VIX_PERCENTILE_RUNGS}
...
def get_thresholds() -> dict[float, float] | None:
    """Ascending percentile -> VIX-level thresholds from cache, or None if
