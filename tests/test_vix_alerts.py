...
def test_buy_skip_due_to_flag_off_is_suppressed():
    suppress, reason = vix_alerts.should_suppress("BUY_SVIX_RUNG", False, "ENABLE_VIX_AUTO_BUY=false")
...
def test_buy_skip_for_a_different_reason_is_not_suppressed():
    ...
    suppress, reason = vix_alerts.should_suppress("BUY_SVIX_RUNG", False, "sleeve % cap would be exceeded")
    assert suppress is False


def test_executed_action_is_never_suppressed():
    suppress, reason = vix_alerts.should_suppress("BUY_SVIX_RUNG", True, "")
    assert suppress is False
